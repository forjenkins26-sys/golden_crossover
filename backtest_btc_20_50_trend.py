"""
BTC 20/50 SMA with 200-day Trend Filter - Test B
Only enters long when price > 200 SMA (bull regime confirmation)
"""
...
class BTC20_50_Trend_Filter(Strategy):
    """20/50 SMA with 200 SMA trend filter - Long only"""
    ...
    def next(self):
        # Golden Cross: 20 > 50, AND price > 200 MA (bull regime)
        if crossover(self.ma20, self.ma50):
            if not self.position and self.data.Close[-1] > self.ma200[-1]:
                self.buy()
        
        # Death Cross: 20 < 50, OR price drops below 200 MA
        elif crossover(self.ma50, self.ma20):
            if self.position:
                self.position.close()
        elif self.position and self.data.Close[-1] < self.ma200[-1]:
            self.position.close()
...
def run_backtest(df):
    ...
    print("\n" + "=" * 80)
    print("BACKTEST CONFIGURATION - TEST B: BTC 20/50 with 200 Trend Filter")
    print("=" * 80)
    print(f"Initial Cash:       $100,000")
    print(f"Commission:         0.1% (0.001)")
    print(f"Strategy:           20/50 SMA Golden Cross + 200 SMA Trend Filter")
    print(f"Entry Rules:        Golden cross (20>50) AND price > 200 SMA")
    print(f"Exit Rules:         Death cross (20<50) OR price < 200 SMA")
...
def print_stats(stats, df):
    ...
    print("\n" + "=" * 80)
    print("BACKTEST RESULTS: BTC 20/50 with 200 Trend Filter (2021-2025)")
    print("=" * 80)
...
if __name__ == '__main__':
    print("\n" + "=" * 80)
    print("BTC 20/50 with 200 TREND FILTER BACKTEST (2021-2025)")
    print("=" * 80 + "\n")
