"""
CURIOSITY TEST: Bidirectional Strategy without Session Filtering
March 1 - April 10, 2026 Backtest
Allows trades during OFF-SESSION hours to compare P&L impact
"""
...
# ============================================================================
# SESSION FILTERING (still tracking for reference, but NOT enforcing)
# ============================================================================
...
print("=" * 150)
print("CURIOSITY TEST: NO SESSION FILTERING (Allows Off-Session Trades)")
print("March 1 - April 10, 2026")
print("Flat Lot Sizing (0.10 BTC) + RSI + Bollinger Bands only")
print("=" * 150)
...
for idx in range(len(df)):
    ...
    # Entry signals (NO SESSION FILTERING - trades allowed anytime)
    if not open_trade and rsi is not None:
...
# Results
print("\n" + "=" * 150)
print("BACKTEST RESULTS - NO SESSION FILTERING")
print("=" * 150)
...
off_session_trades = sum(1 for t in trades if t['Session_In'] == 'Off-Session')
...
print(f"  Off-Session Trades: {off_session_trades}")
...
# Save to CSV
if trades:
    output_file = "backtest_flat_no_session_filter.csv"
...
# Comparison with session-filtered version
print("\n" + "=" * 150)
print("COMPARISON: With Session Filter vs Without")
print("=" * 150)
print(f"With Session Filter:    18 trades | +$461.17 | +92.23%")
print(f"No Session Filter:      {total_trades} trades | ${total_pnl:+.2f} | {return_pct:+.2f}%")
if total_trades > 18:
    print(f"\n  Additional trades from off-session: {total_trades - 18} trades")
    print(f"  Impact on returns: {return_pct - 92.23:+.2f} percentage points")
print("=" * 150)
